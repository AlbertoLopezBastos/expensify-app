...
test('should edit expense on firebase', (done) => {
  ...
  store.dispatch(startEditExpense(id, newExpense)).then(() => {
    ...
    return database.ref(`users/${uid}/expenses/${id}`).once(newExpense);
  }).then((snapshot) => {
    expect(snapshot.val().description).toBe(newExpense.description);
    done();
  });
});
...
test('should fetch the expenses from firebase', (done) => {
  const store = createaMockStore(defaultAuthState);
